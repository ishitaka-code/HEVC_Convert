...
# プログラムのパス
path_NVEncC64 = os.path.abspath(dirpath_this + r"\bin\NVEncC_7.06_x64\NVEncC64.exe")
...
while True:
    try:
        print("--vbr-quality(qvbr)の目標品質値を入力して下さい．指定可能範囲：0～51")
        print("ソフトを終了するときは-1を入力して下さい．")
        hevc_q = int(input("目標品質値 = "))
        if 0 <= hevc_q <= 51:
            break
        elif hevc_q == -1:
            sys.exit()
...
for inputfile in filenames:
    ...
    # NVEncCで変換
    print("NVEncC.exe開始：" + str(datetime.datetime.now()))
    temp_hevcfile = inputfile + "_temp.hevc"
    cmd_string = "\"{path_ffmpeg}\" -loglevel warning -i \"{inputfile}\" -an -sn -f yuv4mpegpipe - | " \
                 "\"{path_NVEncC64}\" --y4m --input - " \
                 "-c hevc -u quality --vbr 0 --multipass 2pass-full --profile main10 --output-depth 10 " \
                 "--vbr-quality {hevc_q} --qp-init 20:23:25 --lookahead 32 --gop-len auto " \
                 "--weightp --level 6 --bref-mode each --output \"{temp_hevcfile}\"".format(**locals())

    subprocess.run(cmd_string, shell=True)

    # ffprobeでJSON形式データ取得
    ffprobe_cmd = "\"{path_ffprobe}\" -i \"{inputfile}\" -loglevel quiet -show_streams " \
                  "-print_format json".format(**locals())
    ...
    outputfile = f_splitpath + " [NVEnc_HEVC10bit_q" + str(hevc_q) + "].mkv"
